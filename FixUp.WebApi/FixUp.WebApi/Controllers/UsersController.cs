...
//עמוד זה לא רלוונטי כי אין משתמש לבד יש או לקוח או בעל מקצוע שיורשים מיוזר
...
//        // 4. התחברות - Login
//        [HttpPost("login")]
//        public async Task<ActionResult<UserDto>> Login([FromBody] UserLoginDto loginDto) // ודאי שכתוב כאן UserLoginDto ולא User
//        {
//            var user = await _userService.Authenticate(loginDto);
//            if (user == null) return Unauthorized("שגיאה במייל או בסיסמה");
//            return Ok(user);
//        }
//        // 5. עדכון משתמש (גנרי)
//        [HttpPut("{id}")]
